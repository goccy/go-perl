...
//	                   cc -shared -fPIC -O2 -I <go-perl>/xsnative/sdk/include \
//	                      -I <dist>/src -I <dist> \
...
func TestTextXslate(t *testing.T) {
	...
	if err := xsnative.Load(p, "Text::Xslate", so); err != nil {
		t.Fatalf("Load: %v", err)
	}
	...
	if r, err := p.Eval(ctx, `sub __t_xinc { unshift @INC, @_; 1 } 1;`); err != nil || !r.Ok {
	...
	mustEval := func(what, src, want string) {
		...
		if want != "" && r.Result != want {
			t.Fatalf("%s = %q, want %q", what, r.Result, want)
		}
	}
	...
	`)
	if err != nil || !r.Ok {
		t.Fatalf("error path: err=%v ok=%v error=%q", err, r.Ok, r.Error)
	}
	if !strings.HasPrefix(fmt.Sprint(r.Result), "died: Undefined symbol") {
		t.Fatalf("error path = %q, want a died: Undefined symbol message", r.Result)
	}
