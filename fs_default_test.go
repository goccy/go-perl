...
// state. Tools that want perl-like behavior opt in with HostFS.
func TestDefaultFSIsSandboxed(t *testing.T) {
	...
	if err != nil || !r.Ok {
		t.Fatalf("host visibility probe: err=%v error=%q", err, r.Error)
	}
	if r.Result != "hidden" {
		t.Fatalf("sandboxed instance sees host file: %q", r.Result)
	}
	...
	`)
	if err != nil || !r.Ok {
		t.Fatalf("private write/read: err=%v error=%q", err, r.Error)
	}
	if r.Result != "private" {
		t.Fatalf("private FS round trip = %q", r.Result)
	}
	...
	if err != nil || !r.Ok {
		t.Fatalf("isolation probe: err=%v error=%q", err, r.Error)
	}
	if r.Result != "isolated" {
		t.Fatalf("instances share filesystem state: %q", r.Result)
	}
}

// TestHostFSOptIn pins the perl-like mode gperl uses.
func TestHostFSOptIn(t *testing.T) {
	...
	`)
	if err != nil || !r.Ok {
		t.Fatalf("host read/write: err=%v error=%q", err, r.Error)
	}
	if r.Result != "from host" {
		t.Fatalf("host read = %q", r.Result)
	}
