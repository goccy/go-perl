...
func TestTimeHiResRealXS(t *testing.T) {
	...
	`)
	if err != nil || !r.Ok {
		t.Fatalf("Time::HiRes eval: err=%v ok=%v error=%q", err, r.Ok, r.Error)
	}
	parts := strings.Split(r.Result, "|")
	if len(parts) != 6 {
		t.Fatalf("unexpected result shape: %q", r.Result)
	}
	...
	if err != nil || !r.Ok {
		t.Fatalf("VERSION: err=%v error=%q", err, r.Error)
	}
	if r.Result != "xs-booted" {
		t.Fatalf("VERSION probe = %q", r.Result)
	}
}
