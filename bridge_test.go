...
func TestObjectIdentityRoundTrip(t *testing.T) {
	...
	if r, err := p.Eval(ctx, `$counter->{n}`); err != nil || r.Error != nil || r.Value.String() != "1" {
		t.Fatalf("Perl-side n after Go inc = %q (err=%v error=%v)", r.Value.String(), err, r.Error)
	}
...
func TestBindReceivesRefs(t *testing.T) {
	...
	if r.Value.String() != "Box" {
		t.Fatalf("bound fn saw class %q, want Box", r.Value.String())
	}
	if kept == nil {
...
func TestBindGoFunction(t *testing.T) {
	...
	if r.Error != nil || r.Value.String() != "HELLO" {
		t.Fatalf("go_upper = ok=%v result=%q error=%v", (r.Error == nil), r.Value.String(), r.Error)
	}
}
...
func TestBindErrorBecomesDie(t *testing.T) {
	...
	if r.Error != nil || !strings.Contains(r.Value.String(), "intentional failure") {
		t.Fatalf("captured $@ = %q (ok=%v error=%v)", r.Value.String(), (r.Error == nil), r.Error)
	}
}
...
func TestGoFuncValueAsCallback(t *testing.T) {
	...
	r, err := p.Eval(ctx, `
		...
		1;`)
	if err != nil || r.Error != nil {
		t.Fatalf("define helpers: err=%v error=%v", err, r.Error)
	}
...
func TestBindClassFromGo(t *testing.T) {
	...
	if err != nil || r.Error != nil || r.Value.String() != "QUIET" {
		t.Fatalf("instance method = %q (err=%v error=%v)", r.Value.String(), err, r.Error)
	}
	r, err = p.Eval(ctx, `Shout->who`)
	if err != nil || r.Error != nil || r.Value.String() != "class:Shout" {
		t.Fatalf("class invocant = %q (err=%v error=%v)", r.Value.String(), err, r.Error)
	}
	...
	if err != nil || r.Error != nil || r.Value.String() != "SUB" {
		t.Fatalf("inherited method = %q (err=%v error=%v)", r.Value.String(), err, r.Error)
	}
	r, err = p.Eval(ctx, `Louder->new->who`)
	if err != nil || r.Error != nil || r.Value.String() != "instance:Louder" {
		t.Fatalf("subclass invocant = %q (err=%v error=%v)", r.Value.String(), err, r.Error)
	}
}
...
func TestRefKeepsPerlValueAlive(t *testing.T) {
	...
	if r, err := p.Eval(ctx, `undef $main::t; $Tracked::destroyed`); err != nil || r.Error != nil || r.Value.String() != "0" {
		t.Fatalf("destroyed after undef = %q (err=%v error=%v), want 0", r.Value.String(), err, r.Error)
	}
	...
	if r, err := p.Eval(ctx, `$Tracked::destroyed`); err != nil || r.Error != nil || r.Value.String() != "1" {
		t.Fatalf("destroyed after Free = %q (err=%v error=%v), want 1", r.Value.String(), err, r.Error)
	}
...
func TestRefFinalizerReleasesPins(t *testing.T) {
	p := newPerl(t)
	ctx := context.Background()
	if r, err := p.Eval(ctx, `sub fresh_ref { { n => $_[0] } } 1;`); err != nil || r.Error != nil {
		t.Fatalf("define fresh_ref: err=%v error=%v", err, r.Error)
	}
...
func TestCallbackDoesNotDeadlockOtherGoroutines(t *testing.T) {
	...
	go func() {
		...
		if err == nil && (r.Error != nil || r.Value.String() != "42") {
			err = fmt.Errorf("nested eval ok=%v result=%q error=%v", (r.Error == nil), r.Value.String(), r.Error)
		}
		otherDone <- err
	}()
	r, err := p.Eval(ctx, `go_wait()`)
	if err != nil || r.Error != nil || r.Value.String() != "released" {
		t.Fatalf("go_wait = ok=%v result=%q error=%v err=%v", (r.Error == nil), r.Value.String(), r.Error, err)
	}
}
